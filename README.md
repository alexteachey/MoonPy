...
For example: Start by identifying the path to your python libraries by typing ```which python```. On my machine, for example, this is ```/opt/anaconda3/bin/python```. With this information you figure out where to place the ```moonpy.pth``` file. Using my path as an example, you will want to place ```moonpy.pth``` within ```/opt/anaconda3/lib/python3.7/site-packages```. Your path may well be different. If you use anaconda environemts, it may be something like ```~/anaconda3/envs/{NAME_OF_ENVIRONMENT}/lib/pythonX.X/site-packages/```. 
